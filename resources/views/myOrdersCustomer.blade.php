            ...
            <div class="orders-container">
                @if(count($executors) !== 0)
                    @foreach($executors as $exe)
                        ...
                        <div class="order" data-task-id="{{$exe->task_id}}"
                             data-exe-id="{{$exe->executor_id}}"
                             data-order-id="{{$exe->id}}"
                             data-cust-id="{{$exe->customer_id}}">
                            <div class="title-price">
                                <a href="{{route('singleTask',$exe->task_id)}}"
                                   class="order-title">{{$exe->task()->title}}</a>
                                <span class="order-price">{{$exe->task()->money()}} </span>
                            </div>

                            <div class="order-address">{{$exe->task()->place}}</div>
                            ...
                            <div class="order-down">
                                <div class="order-author">
                                    <div class="order-author__image">
                                        @if($exe->executor()->image===null)
                                            <img src="{{asset('public/assets/avatars/default.png')}}" alt="Фото профиля">

                                        @else
                                            <img src="{{$exe->executor()->getImageUrlAttribute()}}" alt="Фото профиля">
                                ...
                                @if($exe->status === 'finished')
                                    <div class="finished">
                                        ...
                                        @if(count($feedbacksm) === 0)
                                            <div class=" orderChangeStatus"> Оставить отзыв</div>

                                        @else
                                           @if(!in_array($exe->id,$feedbacksm))
                                                <div class=" orderChangeStatus"> Оставить отзыв</div>
